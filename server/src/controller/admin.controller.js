...
export const getAdminOrders = async (req, res, next) => {
  ...
  try {
    const orders = await Booking.find({
      ...(sanitizeQuery && { $or: [{ userId: { $in: matchUserIds } }] }),
      ...
      ...(sanitizeStartDate || sanitizeEndDate
        ? {
            "pickUp.date": {
              ...
              ...(sanitizeEndDate && {
                $lte: new Date(sanitizeEndDate(23, 59, 59, 999)),
              }),
            },
          }
        : {}),
...
export const updateOrderDelivery = async (req, res, next) => {
  ...
  try {
    if (!bookingId) {
      return next(responseHandler.errorResponse("Booking id is required", 400));
    }
    const booking = await Booking.findById(bookingId);
    if (!booking) {
      return next(responseHandler.errorResponse("Booking not found", 404));
    }
    // check if order is paid
    if (!booking.isPaid) {
      return next(
        responseHandler.errorResponse(
          "please update payment status first before completing delivery",
          400
        )
      );
    }
...
export const updatePaymentStatus = async (req, res, next) => {
  ...
  try {
    ...
    if (booking.isPaid) {
      return next(
        responseHandler.errorResponse(
          "payment as already been made for this booking",
          400
        )
      );
    }
