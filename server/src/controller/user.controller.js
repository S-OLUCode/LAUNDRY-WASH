...
export const forgotPassword = async (req, res, next) => {
  ...
  try {
    ...
    await user.save();
    process.nextTick(() => {
      mailService.sendPasswordEmail(user, resetPasswordLink);
    });
    return responseHandler.successResponse(
      res,
      null,
    ...
    if (user.passwordTokenExpires < new Date()) {
      ...
      return next(
        responseHandler.errorResponse(
          "Password token has expired, please request  new one",
          400
        )
      );
    }
...
export const resendVerifyToken = async (req, res, next) => {
  ...
  try {
    ...
    return responseHandler.successResponse(
      res,
      null,
      "verification token sent to you email"
    );
  } catch (error) {
    next(error);
  }
...
export const verifyAccount = async (req, res, next) => {
  try {
    ...
    if (!userId || !verifyToken) {
      return next(
        responseHandler.errorResponse("UserId or verifyToken is missing", 400)
      );
    }
    ...
    if (!user) {
      return next(responseHandler.errorResponse("User not found", 404));
    }
    ...
    if (user.verifyTokenExpires < Date.now()) {
      ...
      return next(
        responseHandler.errorResponse("Verification token expired", 400)
      );
    }
